...
const {
  ...
  certificateCheck,
  linkUser,
} = require("../test3rdApi/testApi.controller");
...
exports.createBeat = async (req, res) => {
  ...
  try {
    ...
    if (user.credit < 1) {
      return res.status(400).json({ message: "In sufficient credit" });
    }
    ...
    if (user.credit > 0) {
      try {
        const registerDone = await certification(
          audioFile,
          beatName,
          excerpt,
          tags,
        ...
        if (registerDone) {
          ...
          const newBeat = await Beat.create({
            ...
            youtubeUrl,
            register,
            registerCode: registerDone?.workcode,
            registrationStatus: registerDone?.workprivateGet?.status,
...
const certification = async (audio, beatName, excerpt, tags, title, user) => {
  ...
  // Upload the file
  const uploadTicket = await uploadFile(
    uploadurl,
    uploadid,
    path,
    originalname,
  ...
  const workRegisterrrr = await workRegister(
    uploadTicket,
    nonckeyGet,
    excerpt,
    tags,
  ...
  const workcode = workRegisterrrr?.workregistry?.code[0] || null;
...
exports.deleteBeat = async (req, res) => {
  ...
  if (deletedBeat.deletedCount > 0) {
    ...
    return res
      .status(500)
      .json({ message: "No beat found with the specified ID.", error });
  }
};
